...
public class SplittingTheLootScript : MonoBehaviour
{
    ...
    public SpriteRenderer[] BagRenderers;
    public Material[] BagMats;
    public Sprite[] BagSprites;
    public TextMesh[] BagTxts;
    ...
    void Activate()
    {
        ...
        {
            var index = x;
            BagTxts[index].text = bags[index].Label;
            BagRenderers[index].material = ColorToMaterial(bags[index].Color);
            BagRenderers[index].sprite = ColorToSprite(bags[index].Color);
            Bags[index].OnInteract += delegate
            {
                Bags[index].AddInteractionPunch();
                Audio.PlaySoundAtTransform(bags[index].Type == BagType.Diamond ? "DiamondSound" : "MoneySound",
                    Bags[index].transform);
...
#pragma warning restore 414
    ...
    public IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        if (match.Success)
        {
            ...
            var numbers = match.Groups[1].Value.Split(' ').Select(int.Parse).ToList();
            var color = StringToColor(match.Groups[3].Value);

            if (numbers.Any(x => x < 1 || x > 7))
            {
                yield return "sendtochaterror Check your command! I don't understand it!";
                yield break;
            }
            ...
            if (this.bags.Any(x => x.IsReadOnly && numbers.Contains(x.Index)))
            {
                ...
                yield return "sendtochaterror Don't try to cheat! You are not allowed to change the color of the initially colored bag!";
                yield break;
            }
